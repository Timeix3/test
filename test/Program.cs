...
internal class Program
{
    private static void Main()
    {
        ...
        for (int i = 0; i < lines.Length/3; i++)
        {
            ...
            else profit.Add(index, int.Parse(lines[3 * i + 1].Split().Last()));
            node.SetBorders(int.Parse(lines[i * 3 + 2].Split().First()), int.Parse(lines[i * 3 + 2].Split().Last()));
        }

        profit = profit.OrderByDescending(x => x.Value).ToDictionary(x => x.Key, x => x.Value);
